...
Aplot::Aplot(float win,float hin)
{
xdata.resize(0);
ydata.resize(0);
lt.resize(0);
...
cout<<"# Aplot object defined on canvas : "<<win<<" X "<<hin<<" inch "<<endl;
...
float win=3;
float hin=2;
cout<<"# Aplot object on canvas "<<win<<" X "<<hin<<" inch "<<endl;
xdata.resize(0);
ydata.resize(0);
lt.resize(0);
pt.resize(0);
...
void Aplot::set_title(string lab,string col, string c)
{
ostringstream oss;oss.clear();oss.str("");
...
void Aplot::set_xlabel(string lab, string off, float xoff,float yoff,string colo,string c)
{
ostringstream oss;oss.clear();oss.str("");
oss<<"{\\textcolor[rgb]{"<<tcolor(c)<<"}{"<<lab<<"}}";
...
void Aplot::set_ylabel(string lab,string off, float xoff,float yoff,string colo,string c)
{
ostringstream oss;
oss.clear();oss.str("");
oss<<"{\\textcolor[rgb]{"<<tcolor(c)<<"}{"<<lab<<"}}";
...
void Aplot::set_label(string lab, string at_or_at_screen, float atx,float aty,string rot,float rotdeg, string dum, string c)
{
ostringstream oss;oss.clear();oss.str("");
oss<<"{\\textcolor[rgb]{"<<tcolor(c)<<"}{"<<lab<<"}}";
...
void Aplot::set_xtics(float x1,float dx, float x2)
{
fout<<"set xrange ["<<x1<<":"<<x2<<"]"<<endl;
fout<<"set xtics "<<x1<<","<<dx<<","<<x2<<endl;
}
void Aplot::set_ytics(float y1,float dy, float y2)
{
fout<<"set yrange ["<<y1<<":"<<y2<<"]"<<endl;
fout<<"set ytics "<<y1<<","<<dy<<","<<y2<<endl;
}
void Aplot::set_key(string tb,string lr,string bn,string ht, float h)
{
fout<<"set key "<<tb<<"  "<<lr<<" "<<bn<<"  height "<<h<<endl;
}
void Aplot::set_command(string com)
{
fout<<com<<endl;
}
void Aplot::set_data(vector<float> x,vector<float> y,string ltlw,int ltt,float lww, string ptps,int ptt,float pss, string colr, string c, string legend,string t)
{
xdata.push_back(x);
ydata.push_back(y);
lt.push_back(ltt);
...
void Aplot::generate_plot(string fn)
{
...
if(pt[i]>0 && ps[i]>0)fout<<"ps "<<pss*ps[i]<<" ";
fout<<" lc rgb '"<<col[i]<<"' ";
fout<<" t '{"<<leg[i]<<"}' ";
...
if(exts[0]=='e')
{
cout<<"#\t generating eps file : "<<fn<<endl;
oss<<"cp tempeps/doc.eps "<<fn;
system(oss.str().c_str());
}
if(exts[0]=='p'&&exts[2]=='f')
{
cout<<"#\t generating pdf file: "<<fn<<endl;
oss<<"cp tempeps/doc.pdf "<<fn;
system(oss.str().c_str());
}

if(exts[0]=='p'&&exts[2]=='g')
{
oss.str(""); oss.clear();
oss<<"pdftoppm tempeps/doc.pdf -f 1 tempeps/conv"<<" -png"; 
system(oss.str().c_str());
cout<<"#\t generating png file: "<<fn<<endl;
oss.str(""); oss.clear();
oss<<"cp tempeps/conv-1.png "<<fn;
system(oss.str().c_str());
}
...
void Anim::generate_plot(Aplot & P,int fno)
{
ostringstream oss; 
oss.str(""); oss.clear();
...
void Anim::generate_animated_gif(string dum1,int delms,string dum2, int loop,string fgif)
{
make_directory_if_do_not_exist(fgif);
